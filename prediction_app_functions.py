...
def timeseries_frequency_plot(title, df, df2, timeseries_col, y_col, dStart, dEnd, sample_by='D', xtick_freq ='W', axvline=[np.nan]):
    ...
    ax.legend(['Revenue', 'Potential loss'], fontsize=14, loc="upper right")
...
def daily_loss(df):
    ...
    AVG_DAILY_LOSS = -1*daily_losses.set_index('projected_arrival').resample('D')['cost_of_stay'].sum().mean()
